  ...
  drawSummaryCI <- function(CI.L, ES, CI.U) {
    grid::grid.polygon(x = grid::unit(c(CI.L, ES, CI.U, 
                                        ES), "native"), y = grid::unit(0.5 + c(0, 0.25, 
                                                                               0, -0.25), "npc"), gp = grid::gpar(fill = "green"))
  }
...
ex_hpc_data <- read_csv("Hippocampus_MetaAnalysis_Agreement_BDNF_2023017.csv", 
...
ex_hpc_data_smd_bdnf <- ex_hpc_data %>% 
...
## BDNF
ex_hpc_data_smd_bdnf_wide_1 <- ex_hpc_data_smd_bdnf %>%
  group_by(study_year_int_dpt) %>% 
  mutate(time = seq_along(study_year)) %>% 
  ungroup()

ex_hpc_data_smd_bdnf_wide <- ex_hpc_data_smd_bdnf_wide_1 %>% 
  pivot_wider(id_cols = study_year_int_dpt, names_from = time, values_from = c(smd, var_d))

### Rowmeans
ex_hpc_data_smd_bdnf_wide_rmeans <- ex_hpc_data_smd_bdnf_wide %>% 
  mutate(bdnf_smd = rowMeans(ex_hpc_data_smd_bdnf_wide[2],na.rm = TRUE),
         bdnf_var_smd = rowMeans(ex_hpc_data_smd_bdnf_wide[3],na.rm = TRUE))

bdnf_smd_var <- ex_hpc_data_smd_bdnf_wide_rmeans %>% 
  dplyr::select(1,4,5)

# Join the datasets
df_list <- list(ex_hpc_data, hpc_smd_var, crf_smd_var, bdnf_smd_var)
...
png(filename="Plots/hipp_forest.png", width=7, height=10, units = "in", res = 600)
...
png(filename="Plots/crf_forest.png", width=7, height=8.5, units = "in", res = 600)
...
bdnf.smd <- robu(smd~1,var=var_d,studynum=study_year_int_dpt,ex_hpc_data_bdnf) # Clustered at level of overall study
print(bdnf.smd)
sensitivity(bdnf.smd)

# Extract 95% CI
bdnf.smd[["data"]][["ci.u"]] <- bdnf.smd[["data.full"]][["effect.size"]]+(1.96*sqrt(bdnf.smd[["data.full"]][["var.eff.size"]]))
bdnf.smd[["data"]][["ci.l"]] <- bdnf.smd[["data.full"]][["effect.size"]]-(1.96*sqrt(bdnf.smd[["data.full"]][["var.eff.size"]]))
bdnf.smd[["data"]][["ci"]] <- paste("[",format(round(bdnf.smd[["data"]][["ci.l"]], 3), nsmall = 3), "; ", 
                                         format(round(bdnf.smd[["data"]][["ci.u"]], 3), nsmall = 3), "]", sep = "")

bdnf.smd[["data"]][["study_year_int_dpt"]] <- bdnf.smd[["data"]][["study_year_int_dpt"]] %>% 
  str_replace_all("_", " ") %>% 
  str_replace_all("AT CON Baseline", "") %>% 
  str_replace_all(" CON Baseline Year 1", "") %>% 
  str_replace_all("  3mo", "") %>% 
...
bdnf.smd[["data"]][["measure"]] <- bdnf.smd[["data"]][["measure"]] %>% 
  str_replace_all("_", " ")

png(filename="Plots/bdnf_forest.png", width=7, height=6, units = "in", res = 600)
bdnf <-forest.robu_mod(bdnf.smd,es.lab="measure",study.lab="study_year_int_dpt",
                         "SMD" = effect.size,
                         "95% CI" = ci,
                         "Weight" = r.weights)
print(crf)
...
crf_v_hpc <- ggplot(aes(x = crf_smd, y = hpc_smd, size = (1/hpc_var_smd)), data = hpc_crf_bdnf_d_var) + geom_point(aes(colour = study_year_int_dpt)) +
...
#ggsave(crf_v_hpc, height= 10, width= 14, file="crf_predict_hpc_20230118.jpeg", dpi = 300)
...
# Funnel plots
png(filename="Plots/hipp_funnel.png", width=5, height=5, units = "in", res = 600)
...
png(filename="Plots/crf_funnel.png", width=5, height=5, units = "in", res = 600)
